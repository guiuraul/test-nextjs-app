...
function LoadingCard({ className }: { className?: string }) {
  return <div className={["animate-pulse rounded-2xl bg-white/5", className].filter(Boolean).join(" ")} />;
}
      ...
      <div className="mx-auto flex min-h-[calc(100vh-3rem)] w-full max-w-7xl flex-col gap-8">
        ...
        <main className="space-y-6 pb-2">
          ...
          <section className="space-y-3">
            <LoadingCard className="h-8 w-56 rounded-full" />
            <LoadingCard className="h-[30rem] rounded-[2rem] border border-white/10 bg-slate-950/45" />
          </section>
        </main>
